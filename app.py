...
# === HTML保存 ===
def save_html(image_base64, comp, bright, sharp, comment, filename):
...
def load_all_comments():
    ...
    c = conn.cursor()
    c.execute("""
        SELECT id, filename, composition, brightness, sharpness, gpt_comment, timestamp
...
def main():
    ...
    if uploaded:
        ...
        if st.button("📄 HTMLに保存"):
            html_path = save_html(image_base64, comp, bright, sharp, comment, filename)
            st.success(f"HTML保存完了: {html_path}")
